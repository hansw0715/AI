...
Panda3D 1인칭 슈터. 원래는 좀비 웨이브 캠페인(반전: 플레이어=바이러스, 적=정상 로봇)으로
시작했고, 이후 여러 솔로/멀티 모드가 추가됨. 이 파일은 claude.ai 프로젝트에 올려 전체
코드를 한 번에 참고하기 위한 단일 번들이다.

## 게임 모드 (메뉴: 솔로플레이 / 멀티플레이)
- **웨이브(캠페인)**: 좀비 생존 (build_level). 솔로 기본.
...
- `level.py` — 맵 빌드 — build_level/arena/soccer_field/paint_field/jump_field + 충돌
