# AI Shooter — Claude Project Bundle

학교 그룹 프로젝트 (`hansw0715/AI` GitHub, branch `2693044한승원`).
Panda3D 기반 **발로란트 스타일 1인칭 택티컬 슈터**. 솔로(AI 봇) / 멀티(온라인 1:1)
대전과 몇 가지 미니게임 모드로 구성된다. 이 파일은 claude.ai 프로젝트에 올려 전체
코드를 한 번에 참고하기 위한 단일 번들이다.

> **방향 정리(폐기됨)**: 초기의 "좀비 웨이브 생존" 캠페인과 "플레이어=바이러스 / 적=정상
> 로봇" 반전 컨셉, 그리고 그에 딸린 글리치 HUD 연출은 **전부 폐기**했다. 더 이상 게임의
> 컨셉이 아니다. (코드 파일명 `zombie_game.py` 등은 히스토리상 그대로지만 의미 없음.)

## 게임 모드 (메뉴: 솔로플레이 / 멀티플레이)
- **AI 대결**: 아레나에서 소총 AI 봇과 1:1 (ai_mode).
- **1:1 PvP**: 릴레이 기반 온라인 1:1, 먼저 10킬 (online_mode, build_arena).
- **축구**: 총알로 공을 차서(마그누스 커브) 골대에 넣기. 먼저 5골 승리. 1점 득실마다
  스폰 복귀 + 5초 카운트다운. 공-몸 솔리드 충돌. 솔로(봇 공/플레이어 번갈아 사격)/멀티.
  (soccer_mode, build_soccer_field, SoccerBall)
- **땅따먹기(영역 페인트)**: 바닥/벽 격자 셀을 칠해 3분 뒤 더 많이 칠한 쪽 승리. 킬로 칠하기,
  적 처치 시 시체 위 힐팩. (paint_mode, build_paint_field, 단일 Geom 정점색)
- **점프맵(레이스)**: 평행 레인 점프 코스(벽 우회/빔/지그재그). 자율 AI 봇(웨이포인트 진행 +
  견제 사격). 낙하 시 시작점 복귀, 결승 먼저 도달 승리. (jump_mode, build_jump_field)

## UI / HUD 방향 — 깔끔한 발로란트 스타일
- 목표는 **발로란트처럼 미니멀하고 깔끔한 HUD**. 화면을 가리지 않는 얇은 선·작은 폰트·
  절제된 컬러로, 정보(체력/탄약/킬/타이머)만 또렷하게 읽히게 한다.
- **제거된 옛 컨셉**: "백신 AI 위장 → 글리치 시 빨강으로 진실 노출" 연출, 바이러스/병변
  테마 색, 임상(antivirus) 톤 등은 **전부 폐기**. 더는 컬러를 컨셉 연출에 쓰지 않는다.
- 새 톤: 발로란트풍 중립 다크 + 단일 액센트 컬러, 명확한 크로스헤어, 깔끔한 킬/탄약 표시.
  HUD 색상 상수와 글리치 라벨 토글은 이 새 방향에 맞춰 단순화할 예정.

## 구조 요점
- 모드 플래그: `online_mode / ai_mode / soccer_mode / paint_mode / jump_mode`.
  `_start_game(online, ai, soccer, paint, jump)` → `_build_world()` 가 맵 선택, `_setup_*()` 셋업.
- 맵: `level.py` 의 `build_*` 함수가 `(LevelCollider, arena_data)` 반환. arena_data 에 spawns/
  platforms/배리어 + 모드별 데이터(soccer/paint/jump).
- 네트워크: Fly 릴레이로 raw 바이트 중계. `NET_STATE_FMT` 고정 프레임(위치/무기/발사/피해/
  사망/nonce/이름 + 축구 공·점수·킥 이벤트). 스폰은 nonce 비교로 자동 A/B 배정.
- 슬로우모션(매치 종료), 데미지 플래시(체력바), 히트마커, 킬배너, 부활 무적 링 등 피드백.
- 직접실행 플래그: `--online / --p2 / --soccer / --paint / --jump`.

---

## 목차
- `zombie_game.py` — 메인 — 게임 루프/모드/플레이어/HUD/네트워크/AI/모든 모드 로직
- `level.py` — 맵 빌드 — arena/soccer_field/paint_field/jump_field + 충돌
- `weapon_config.py` — 무기 배치/스탯 상수
- `kit_map.py` — (옵션) Quaternius sci-fi 키트 벽 메쉬 시각 레이어

---

## File: `zombie_game.py`
메인 — 게임 루프/모드/플레이어/HUD/네트워크/AI/모든 모드 로직
...
## File: `level.py`
맵 빌드 — build_level/arena/soccer_field/paint_field/jump_field + 충돌
...
## File: `kit_map.py`
(옵션) Quaternius sci-fi 키트 벽 메쉬 시각 레이어
