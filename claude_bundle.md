...
- **점프맵(레이스)**: 평행 레인 점프 코스(벽 우회/빔/지그재그). 자율 봇(웨이포인트 진행 +
  견제 사격). 낙하 시 시작점 복귀, 결승 먼저 도달 승리. (jump_mode, build_jump_field)

## UI / HUD — 깔끔한 발로란트 스타일
- 발로란트처럼 미니멀하고 깔끔한 HUD. 얇은 선·작은 폰트·절제된 컬러로 정보(체력/탄약/
  킬/타이머)만 또렷하게. 중립 다크 + 단일 레드 액센트.
- 메뉴/타이틀/일시정지/설정/멀티 로비/솔로 배치/결산 전부 택티컬(레드·블랙 노치 패널)
  스타일로 통일. 화면 전환은 좌→우 슬라이드 + 페이드.
...
> 참고: 파일명 `zombie_game.py` 와 일부 내부 식별자(예: `Zombie`, `ZOMBIE_RADIUS`)는
> 초기 프로토타입 시절 이름이 남은 것일 뿐 게임 컨셉과 무관하다 — 그냥 '적' 캐릭터다.

---

## File: `zombie_game.py`
메인 — 게임 루프/모드/플레이어/HUD/네트워크/봇/모든 모드 로직
...
## File: `level.py`
맵 빌드 — arena/soccer_field/paint_field/jump_field + 충돌
